...
base_model = MobileNet(input_shape = (img_h, img_w, 3), include_top = False, weights = 'imagenet')
...
model.summary()
model.compile(optimizer = tf.keras.optimizers.Adam(lr=0.0001) , loss = 'categorical_crossentropy',metrics = ['accuracy',AUC(), precision_m,f1_m,recall_m])



from tensorflow.keras.callbacks import ModelCheckpoint, LearningRateScheduler, EarlyStopping, ReduceLROnPlateau
weight_path="{}_weights.best.hdf5".format('cvs.mobilenet')
...
#model = keras.models.load_model('inception_resnet.h5')
pred = model.predict(test_X, batch_size = 300, verbose = 2)
